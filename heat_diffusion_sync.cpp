...
double parallel(vector<vector<double>> &mat, int rsize, int csize, int world_rank, int world_size) {
    ...
    for (int step = 1; step <= NUM_ITER; ++step) {
        ...
        if (world_rank > 0)
            MPI_Irecv(local[0].data() + PAD_SIZE, csize, MPI_DOUBLE, world_rank - 1, 0, MPI_COMM_WORLD, &reqs[req_count++]);

        // Receive from bottom neighbor
        if (world_rank < world_size - 1)
            MPI_Irecv(local[local_rows + PAD_SIZE].data() + PAD_SIZE, csize, MPI_DOUBLE, world_rank + 1, 0, MPI_COMM_WORLD, &reqs[req_count++]);

        // Send top row to upper neighbor
        if (world_rank > 0)
            MPI_Isend(local[PAD_SIZE].data() + PAD_SIZE, csize, MPI_DOUBLE, world_rank - 1, 0, MPI_COMM_WORLD, &reqs[req_count++]);

        // Send bottom row to lower neighbor
        if (world_rank < world_size - 1)
            MPI_Isend(local[local_rows].data() + PAD_SIZE, csize, MPI_DOUBLE, world_rank + 1, 0, MPI_COMM_WORLD, &reqs[req_count++]);
...
int main(int argc, char** argv) {
    ...
    if (world_rank == 0) {
        cout << "Synchronous heat diffusion finished. Max time across ranks: " << max_secs << "s\n";
    }
